...
Podemos apreciar que en el primer caso hay una mayor tendencia a estar en el estado de caracter gris porque este tiene una tasa de diverisficación mayor que el negro. En el segundo caso no hay una tendencia tan clara.

Ahora vamos a ajustar nuestro modelo con datos experimentales de peces de arrecifes de coral. Vamos a comparar el modelo en que las tasas de diverisificación están afectadas por los estados de caracter, vivis en arrecifes de coral frente a no vivir en arrecifes de coral, con otro modelo en donde dichas tasas son independientes (Price et al., 2013).
...
Nuestros coeficientes sugiren que tanto la especiación como la extinción son más altas para peces que viven en arrecifes de coral que par a los que no. No obstante, la probabilidad del modelo más complejo no mejora de manera muy significativa el modelo nulo.
...
Esto nos dice que la probabilidad a posteriori de que las tasas de diversificación en los peces de arrecifes de coral sea mayor que en los que no nos de arrecifes de coral es carcana a 1.
...
Uno de los grandes problemas de los modelos SSE es que hay mucho modelos es que tienen una gran tasa de error de tipo I. Nosotro comparamos el modelo BiSSE por ejemplo frente a un modelo nulo, que normalmente muestra peor ajuste, pero hay mucho modelos alternativos que podrían mostrar un mejor ajuste (Rabosky and Golberg, 2015).
Es decir, que pasa si nuestro modelo nulo es peor que BiSSE pero cualquier otro modelo en donde las tasas de especiación y extinción no son constante si no que varian con el tiempo es mejor que nuestro modelo BiSSE. O quizás nuestras tasas varían en función de otro caracter pero que no estamos incluyendo en el análisis.

Beaulier and O'Meara (2016) desarrollaron el modelo HiSSE para solventar este problema.
Este modelo nos permite considerar la prosibildiad de que la especiació y la extinción varían pero no en función del carácter que estamos considerando en el análisis, sino en funcón de un caracter no medido, o también en función de ambos, nuestro caracter y otro caracter no medido.
...
Preramos nuestros datos (al igual que en corHMM los datos deben estar en formato data.frame).
...
La manera de ajustar un modelo BiSSE aquí es seleccionar la opción hidden.states - FAlse and ajustando lo argumetnos de los parametros turnover=c(1,2) y eps=c(1,2) para indicarle que queremos calcular estos parámetros solo para los estados de caracter de nuestro rasgo bajo estudio.
```{r}
...
Y indicamos que es un modelo de tipo CID-2 indicando en los argumentos turnover y eps c(1,1,2,2). Estos valores corresponden a las filas de la matriz rates.cid2 que hemos creado, es decir, una tasa de diversificación para el estado de caracter A y otra para el B, pero no se asignan para 0 y 1 que son los estados observados.
...
Para hace esto ahora tenemos que ajustar los argumentos turnover y eps al valor c(1,1,2,2,3,3,4,4).
...
Finalmente veremos un modelo en el que la tasa de especiación y/o extinción varian en función un caracter cuantitativo continuo.
Este modelo se llama QuaSSE y está implementado en diversitree. Con este modelo podemos modelizar las tasas de especición y/o extinción con una relación lineal (hay otras funciones disponible) con nuestro rasgo continuo o bien una relación constante, es decir, no están relacionados.
...
Ya casi estamos listos. Recuerdas que hay que indicar la fracción del total de especies muestreadas.
...
Es importante también tener en cuenta que en este modelo, difícil de ajustar, es importante empezar con valores iniciales los más cercanos posibles al óptimo y lo haremos con la función starting.point.

Por desgracia la función starting.pint.quasse no está preparada para tener en cuenta solo una fracción de lo muestreado así que haremos un apaño y usaremos la funciones fit.bd y fitContinuous para conseguir esos valores iniciales.
...
```{r}
anova(cid.mle,variable.lambda=lambda.mle,variable.mu=mu.mle,full.model=full.mle)
```
