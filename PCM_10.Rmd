...
Este modelo nos permite considerar la posibilidad de que la especiación y la extinción varían pero no en función del carácter que estamos considerando en el análisis, sino en función de un caracter no medido, o también en función de ambos, nuestro caracter y otro caracter no medido.

Vamos a ajustar por tanto estos modelos con nuestro data set de peces de arrecifes de coral y el paquete de R HiSSE.
En este paquete en vez de calcular directamente la especiación y la extinción se calcula la fracción de la extinción con respecto a la especiación (epsilon = mu / lambda) y el recambio de especies "turnover" (lamba + mu).
...
Una vez hemos ajustado estos modelos más simples y preliminares, vamos de verdad a ajustar modelos nuevos hasta ahora.
...
En primer lugar cogemos la matriz orginal rates.hisse e indicamos que hay un solo parámetro de transición entre los 4 estados de caracter posible (esto lo hacemos para simplificar la matriz de transición entre estados de caracter).
...
La función plot.hisse.states no es muy flexible. Nos da dos resultados, la reconstrucción del caracter observado y las tasas de especiación or diversificación.
...
Finalmente vamos a ajustar un modelo HiSSE donde las tasas de diversificación dependen de los estados de caracter de nuestro rasgo observado y también de los 4 estados de caracter no observados.
...
Podemos ver que el mejor modelo es el más simple (CID), seguido del BiSSE y CID-2.
...
Claramente podemos rechazar la hipotesis nula de diversificación constante.
